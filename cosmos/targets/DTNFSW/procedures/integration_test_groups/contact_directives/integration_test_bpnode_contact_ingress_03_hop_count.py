...
class integration_test_bpnode_contact_ingress_03(Group):
    """
    Test Group
    """

    def test_bpnode_bundle_ingress_03(self):
        """
        Nominal test
        """
        ...
        try:
            ...
            exp_new_hop_count = deserialized_input_bundle.canon_blocks[0].hop_data.hop_count + 1
            ...
            check_expression(f"{received_bundle.canon_blocks[0].blk_type} == {BlockType.HOP_COUNT}")
            check_expression(f"{received_bundle.canon_blocks[0].blk_num} == {3}")
            ...
            check_expression(f"{received_bundle.canon_blocks[0].hop_data.hop_limit} == {deserialized_input_bundle.canon_blocks[0].hop_data.hop_limit}")
            check_expression(f"{received_bundle.canon_blocks[0].hop_data.hop_count} == {exp_new_hop_count}")

            # Check the payload block
            check_expression(f"{received_bundle.canon_blocks[1].blk_type} == {BlockType.BUNDLE_PAYLOAD}")
            check_expression(f"{received_bundle.canon_blocks[1].blk_num} == {1}")
            ...
            check_expression(f"{received_bundle.canon_blocks[1].crc} == {deserialized_input_bundle.canon_blocks[1].crc}")
